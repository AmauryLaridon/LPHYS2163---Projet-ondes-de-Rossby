...
# Crée deux tableaux de 30 pour 60, l'un avec les valeurs de x et l'autre de y
...
def current_flux(zeta, u, v):
    ...
    for i in range(N):
        for l in range(M):
            current_flux[i, l] = (1/(2*delta_s))*(zeta[i, (l+1) % M]*u[i, (l+1) % M] - zeta[i, l-1] *
                                                  u[i, l-1] + zeta[(i+1) % N, l]*v[(i+1) % N, l] - zeta[i-1, l]*v[i-1, l]) + beta * v[i, l]
